...
<#
.SYNOPSIS
    Server-side cutover from JumpCloud to GCPW on sage-iai.

.DESCRIPTION
    sage-iai is a Windows Server 2022 RDP host for Internet Alliance, in a
    separate JumpCloud org from sage-amg and bound to a single Google
    Workspace domain (inetalliance.net). Workstations connect with
    `sage-iai\<first.last>` over Tailscale. This script:

      1. Installs GCPW + configures it for inetalliance.net sign-in.
    ...
    For sage-amg (Ameriglide / cross-domain), use deploy-gcpw-sage-amg.ps1.
...
Write-Host "deploy-gcpw-sage-iai.ps1 rev $Script:Revision" -ForegroundColor DarkGray
$osInfo = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion"
Write-Host "Windows build: $($osInfo.DisplayVersion) $($osInfo.CurrentBuild).$($osInfo.UBR)" -ForegroundColor DarkGray

# ---------------------------------------------------------------------------
# Hostname guard -- this script is sage-iai-specific.
# ---------------------------------------------------------------------------
if (-not $Force -and $env:COMPUTERNAME -inotmatch '^sage-iai$') {
    Write-Error "Refusing to run on '$env:COMPUTERNAME'. This script is sage-iai-specific. Use -Force to override."
    exit 1
}
...
# sage-iai serves a single Workspace tenant (Internet Alliance).
$AllowedDomains   = "inetalliance.net"

$RdpRegPath       = "HKLM:\System\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp"

# JumpCloud Agent is a single-MSI install. Same ProductCode as sage-amg
# (the agent itself is org-agnostic; org binding is in config, not the MSI).
...
# Existing JumpCloud accounts whose Google emails don't follow the default
# `<sam>@inetalliance.net` rule. None observed on sage-iai at cutover time
# (all enabled JC SAMs were `first.last`). Add to this map if any surface.
$EmailOverrides = @{}

# Local accounts to leave entirely alone -- not JumpCloud-provisioned, no GCPW
# association needed. Snapshot from sage-iai discovery 2026-05-03.
# If any of these turn out to need GCPW sign-in, remove from the list and re-run.
$ManualLocalAccounts = @(
    "Administrator", "Guest", "DefaultAccount", "WDAGUtilityAccount",
    "debbie.singh", "gwyn.johnson", "joe.brown", "joseph.ososki",
    "karin.sheffer", "kevyn.thompson", "laurie.baker", "sage",
    "sean.graham", "zak.roberts", $BackupAdminUser
)
...
function Resolve-GoogleEmail {
    ...
    # Default: lowercased SAM @ inetalliance.net if SAM is `first.last`.
    if ($SamName -match '^[a-zA-Z]+\.[a-zA-Z]+$') {
        return "$($SamName.ToLower())@inetalliance.net"
    }
    return $null
}
...
Write-Host "  sage-iai: JumpCloud -> GCPW cutover" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# ----------------------------------------------------------------------------
Write-Host ""
Write-Host "[3/7] Configuring GCPW for sage-iai..." -ForegroundColor Yellow
...
Write-Host "  sage-iai cutover complete." -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
Write-Host "USER COMMS - paste into the announcement:" -ForegroundColor Yellow
Write-Host ""
Write-Host "  RDP to sage-iai has switched to Google sign-in."
Write-Host ""
...
Write-Host "  with your @inetalliance.net account. After that, your sage-iai"
